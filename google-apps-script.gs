 ...
 * Handles sheet management, synchronization, auto-archiving, retrieval,
 * and Obantoko Ward Calendar & Notification System.
 *
    ...
    switch (action) {
      ...
      case "getMetadata":
        return jsonResponse_({
          ok: true,
          status: "success",
          data: { message: "pong", db_version: getDbVersion_() },
          metadata: { last_updated: new Date().toISOString(), db_version: getDbVersion_() },
      ...
      case "import":
      case "pushAll":
        if (action === "pushAll" && payload.data) {
          payload.db = payload.data;
          payload.mode = "merge";
        }
...
function drawLegend(calendarSheet) {
  ...
  for (let i = 0; i < legend.length; i++) {
    ...
    calendarSheet.getRange(row, 16)
      .setBackground(legend[i][0])
      .setValue("")
      .setBorder(true, true, true, true, true, true);

    calendarSheet.getRange(row, 17)
      .setValue(legend[i][1])
      .setFontWeight("bold")
      .setVerticalAlignment("middle")
      .setWrap(false)
  ...
  calendarSheet.setColumnWidth(16, 88);
  calendarSheet.setColumnWidth(17, 320);
