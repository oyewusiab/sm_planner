...
exports.runChecklistReminders = exports.sendNotificationEmail = exports.migrateLegacyUser = exports.adminResetPassword = exports.adminToggleUserStatus = exports.adminDeleteUser = exports.adminCreateUser = void 0;
...
async function assertAdmin(contextAuth) {
    ...
    if (!callerData || callerData.role !== "ADMIN") {
        throw new https_1.HttpsError("permission-denied", "Only administrators can perform this action.");
    }
}
/**
...
exports.adminDeleteUser = (0, https_1.onCall)(async (request) => {
    ...
    if (!user_id) {
        throw new https_1.HttpsError("invalid-argument", "Missing user_id parameter.");
    }
    try {
 ...
 */
exports.adminToggleUserStatus = (0, https_1.onCall)(async (request) => {
    await assertAdmin(request.auth);
...
exports.adminResetPassword = (0, https_1.onCall)(async (request) => {
    ...
    if (!user_id || !password) {
        throw new https_1.HttpsError("invalid-argument", "Missing user_id or password parameters.");
    }
    try {
        // Look up user profile to resolve Auth UID
        const userDoc = await db.collection("users").doc(user_id).get();
        ...
        throw new https_1.HttpsError("internal", error.message || "Failed to reset password.");
    }
});
/**
 * Cloud Function: Migrates a legacy user to Firebase Auth with their existing UID
 */
exports.migrateLegacyUser = (0, https_1.onCall)(async (request) => {
    const { email, password, uid } = request.data;
    if (!email || !password || !uid) {
        throw new https_1.HttpsError("invalid-argument", "Missing email, password, or uid.");
    }
    try {
        // Check if user exists in Firestore
        const userDoc = await db.collection("users").doc(uid).get();
        if (!userDoc.exists) {
            throw new https_1.HttpsError("not-found", "Legacy user profile not found in Firestore.");
        }
        ...
        throw new https_1.HttpsError("internal", error.message || "Failed to migrate user.");
    }
});
 ...
 */
exports.sendNotificationEmail = (0, firestore_1.onDocumentCreated)("notifications/{notificationId}", async (event) => {
    const notif = event.data?.data();
    if (!notif || !notif.to_user_id || !notif.title || notif.read)
        return;
    try {
 ...
 */
exports.runChecklistReminders = (0, scheduler_1.onSchedule)("0 8 * * 5", async () => {
